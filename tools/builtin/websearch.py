...
class WebSearch(Tool):
    ...
    async def execute(self, invocation: ToolInvocation) -> ToolResult:
        ...
        try:
            results = DDGS().text(params.query, region='us-en', safesearch='off', timelimit='y', page=1, backend="auto", max=params.max_results)
        ...
        if not results:
            return ToolResult.success_result(f"No results found for: {params.query}",metadata = {
            'results': 0
        })
